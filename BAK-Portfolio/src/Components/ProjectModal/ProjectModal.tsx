...
const ProjectModal = ({ id, allProjects}: Prop) => {
  ...
  return (
    ...
    >
      {!selectedProject && <h1>Loading...</h1>}
      {selectedProject && 
        <section className='z-51 flex flex-col items-center w-5/6 md:w-1/2 max-h-full border-8 rounded-2xl border-stone-900 bg-stone-100 bg-opacity-100 backdrop-blur-lg px-10 py-6 my-3 drop-shadow-2xl overflow-y-scroll'>
          ...
          {!selectedProject.deployedURL &&  <p className='text-sm md:text-lg font-font5 text-center italic'>Working to get this and remaining studen projects deployed...</p>}
          <p id='backButton' className='hover:cursor-pointer font-font7 text-center rounded-lg border-2 border-stone-800 drop-shadow-2xl p-2 px-4 pt-3 mt-6 mb-2 text-lg md:text-2xl text-teal-400 bg-gradient-to-r from-neutral-800 to-stone-700'>← back</p>
        </section>
      }
    </div>
  )
